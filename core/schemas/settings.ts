...
export const ticketsSettingsSchema = z.object({
  ...
  /**
   * Gmail-style "Undo Send" — when enabled, clicking Send on a reply or forward
   * holds the message for `replyUndoSeconds` before actually sending it,
   * giving the agent a window to recall an in-flight message. The reply is
   * not yet persisted on the server during this window; closing the tab
   * cancels the send (matching the user's mental model of "undoing before
   * delivery").
   */
  replyUndoEnabled:             z.boolean().default(false),
  replyUndoSeconds:             z.number().int().min(3).max(30).default(7),
  // Presence / live collaboration indicators
  presenceEnabled:              z.boolean().default(true),
  // Merge tickets
  mergeTicketsEnabled:          z.boolean().default(true),
  /**
   * AI-powered "Summarize" button on the ticket detail page. When enabled,
   * agents can collapse the entire customer/agent thread into a 2–4
   * sentence brief generated by the AI model. Off-by-default surfaces are
   * misleading for new admins, so default-on; admins can disable to hide
  ...
  //
  // A list of ticket-field keys that must have a non-empty value before an
  // agent can move the ticket into "resolved" / "closed". When violated the
  // server returns a 400 with the missing fields and the agent sees a
  // contextual error in the status picker.
  //
  // Built-in keys: "priority" | "severity" | "impact" | "urgency" |
  //                "category" | "ticketType" | "assignedToId" | "teamId" |
  //                "affectedSystem"
  // Custom fields: prefixed with "cf." — e.g. "cf.root_cause" matches the
  //                CustomField row with `key = "root_cause"`.
  resolveRequiredFields:        z.array(z.string().max(120)).default([]),
  closeRequiredFields:          z.array(z.string().max(120)).default([]),
});
...
export const integrationsSettingsSchema = z.object({
  ...
  // SEPARATE from the Google Meet bridge credentials further down — different
  // scopes, different consent screens.
  googleSignInEnabled:      z.boolean().default(false),
  googleSignInClientId:     z.string().default(""),
  googleSignInClientSecret: z.string().default(""),  // server-only
  // ── Slack ──────────────────────────────────────────────────────────────────
  slackEnabled:       z.boolean().default(false),
  slackWebhookUrl:    z.string().default(""),   // server-only
  // ── Inbound email — chosen transport ───────────────────────────────────────
  /**
   * Selects how inbound customer emails arrive at the helpdesk.
   *  - "webhook"  → SendGrid Inbound Parse (or compatible) POSTs to
   *                 /api/webhooks/inbound-email. Best for production with a
   *                 dedicated MX-routed domain (support@yourdomain.com).
   *  - "imap"     → A pg-boss job polls an IMAP mailbox (Gmail, Outlook, …)
   *                 every minute and ingests unread messages. Best when you
   *                 want to receive on a real Gmail inbox.
   *  - "both"     → Run both. The shared processor de-duplicates by
  ...
  imapPassword:       z.string().default(""),   // server-only — Gmail app password
  /** Use TLS (port 993). Disable for STARTTLS on 143 — rare, mostly self-hosted. */
