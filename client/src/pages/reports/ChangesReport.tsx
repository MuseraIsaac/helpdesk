...
export default function ChangesReport() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <ChartContainer
          config={{ count: { label: "Changes", color: "var(--chart-3)" } }}
          className="h-48"
        >
          <AreaChart data={data.volume}>
            ...
            <Area
              ...
              stroke="var(--chart-3)" fill="url(#changeGrad)" strokeWidth={2}
            />
          </AreaChart>
        </ChartContainer>
      </ChartCard>
          ...
          >
            <BarChart
              data={data.byRisk.map(r => ({
                ...
                fill: RISK_COLORS[r.risk] ?? "var(--chart-4)",
              }))}
              layout="vertical" barSize={14}
            >
              <CartesianGrid horizontal={false} strokeDasharray="3 3" className="stroke-border" />
      ...
      {data.byType.length > 0 && (
        <ChartCard title="Changes by Type" description="Standard vs. emergency vs. pre-approved breakdown.">
          <ChartContainer
            config={{ count: { label: "Count", color: "var(--chart-5)" } }}
            className="h-36"
          >
